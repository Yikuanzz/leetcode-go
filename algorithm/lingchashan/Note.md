...
| [最长递增子序列 LIS](https://www.bilibili.com/video/BV1ub411Q7sB/) | [300. 最长递增子序列](https://leetcode.cn/problems/longest-increasing-subsequence/) | [代码](https://leetcode.cn/problems/longest-increasing-subsequence/solution/jiao-ni-yi-bu-bu-si-kao-dpfu-o1-kong-jia-4zma/) | 包括贪心二分 + ![img](https://cdn.nlark.com/yuque/__latex/a2006f1ac61cb1902beacb3e29fff089.svg) 空间 |
...
相向指针的核心思想是基于线性数据结构（如数组、链表或字符串）的有序性来优化搜素过程，通常用于解决需要在有序数据中查找满足特定条件的元素对问题。



### 两数和

题目地址：[167. 两数之和 II - 输入有序数组](https://leetcode.cn/problems/two-sum-ii-input-array-is-sorted/)

核心思路：利用数组的有序特定，用双指针优化搜索的时间复杂度。



### 三数和

题目地址：[15. 三数之和](https://leetcode.cn/problems/3sum/)

核心思路：发现数组的顺序不影响结果，因此对数组进行排序然后再用双指针优化搜索的时间复杂度。



### 盛最多水的容器

题目地址：[11. 盛最多水的容器](https://leetcode.cn/problems/container-with-most-water/)

核心思路：考虑左右两边的容器边界，发现容器的大小取决于较短的边，所以当容器的容量小于之前的容量容量则需要移动左边界。



接雨水

题目地址：[42. 接雨水](https://leetcode.cn/problems/trapping-rain-water/)

核心思路：

- 1、前缀数组法：考虑当前雨水容量取决于两边最小的值，因此先遍历数组计算当前索引下左边和右边的最大值，然后再逐个计算。
- 2、双指针法：发现容量只与较小值有关，与历史数据无关，所以可以对数组空间进行优化，在一次遍历中对较小值并直接计算容量。



## 滑动窗口

滑动窗口算法的核心思想维护一个动态的”窗口“，可以是连续子数组、子列表或者字符串的子串。通常用于处理序列数据，比如查找满足特定条件的所有连续子序列问题。



### 长度最小的子数组

题目地址：[209. 长度最小的子数组](https://leetcode.cn/problems/minimum-size-subarray-sum/)

核心思路：维护一个子数组窗口，关键是找到满足条件的边界情况，也就是大于目标值的时候，再对数组进行缩减尝试。总体来说就是先将窗口扩大到满足基本条件的情况，再将窗口缩小满足最优条件。



### 无重复字符的最长子串

题目地址：[3. 无重复字符的最长子串](https://leetcode.cn/problems/longest-substring-without-repeating-characters/)

核心思路：维护一个字符窗口大小为 ASCII 表的大小 128，边界条件是出现重复的时候，如果要添加进窗口的字符已经存在则将其删除，然后再添加字符，直到遍历完字符串。



### 乘积小于 K 的子数组

题目地址：[713. 乘积小于 K 的子数组](https://leetcode.cn/problems/subarray-product-less-than-k/)

核心思路：维护一个乘积窗口，边界条件是窗口值大于 `k`，当大于 `k` 时则从数组左边开始除，然后计算数组个数，直到遍历完成。



## 二分查找

二分查找的核心思想是基于分治法和数据有序性来快速缩小搜索范围。



### 在排序数组中查找元素的第一个和最后一个位置

题目地址：[34. 在排序数组中查找元素的第一个和最后一个位置](https://leetcode.cn/problems/find-first-and-last-position-of-element-in-sorted-array/)

核心思想：利用数组的有序性进行搜索，将元素查找目标进行转换。



### 寻找峰值

题目地址：[162. 寻找峰值](https://leetcode.cn/problems/find-peak-element/)

核心思想：因为假设 `nums[-1]` 与 `nums[n]` 为负无穷大，所以对比 `nums[i]` 与 `nums[i+1]`，如果前者大于等于后者，那么从 `0` 到 `i` 必然存在峰值，可以用反证法进行证明。



### 寻找旋转排序数组的最小值

题目地址：[153. 寻找旋转排序数组中的最小值](https://leetcode.cn/problems/find-minimum-in-rotated-sorted-array/)

核心思想：以数组最右边的数为例，它要么是最小值，要么是在最小值的右边，因此判断中间值与数组最后一个数进行比较，如果中间值比最右边的小，那么可能是最小值或者是最小值右边的数。此时最小值的范围应该是从 `0` 开始到 `mid`，因为如果是在 `mid` 到 `n-1` 就不满足原先是有序数组的条件。 



### 搜索旋转排序数组

题目地址：[33. 搜索旋转排序数组](https://leetcode.cn/problems/search-in-rotated-sorted-array/)

核心思想：红蓝染色法思路，蓝色表示该元素小于等于目标值，根据染蓝色的条件去对情况分类，即什么条件下满足 `mid` 染蓝色，以此来进行分支判断。



## 反转链表

反转链表主要是通过改变节点中的指向来实现链表的反转。



### 反转链表

题目地址：[206. 反转链表](https://leetcode.cn/problems/reverse-linked-list/)

核心思路：

- 1、迭代法， 创建一个空的前驱指针，并创建一个临时指针保留链表位置，不断迭代这两个指针完成反转。
- 2、递归法，从末尾节点开始让自己的下个节点指向自己，然后再让自己指向空，先展开到尾部再回到开头。
...
题目地址：[92. 反转链表 II](https://leetcode.cn/problems/reverse-linked-list-ii/)

核心思路：先遍历找到开始反转的位置，因为第1个节点也可能会反转，所以定义一个哑节点来记录位置，之后就是按照反转链表的思路去解决。



### K个一组翻转链表

题目地址：[25. K 个一组翻转链表](https://leetcode.cn/problems/reverse-nodes-in-k-group/)

核心思路：因为不满足 `K` 个则不会进行翻转，所以遍历出链表的长度来决定翻转的组数，然后在每组中翻转 `K` 次，注意更新哑节点。
...
快慢指针的是通过两个指针以不同的速度遍历数据结构（如链表或数组），利用它们的相对运动特性来解决特定问题。



### 链表的中间结点

题目地址：[876. 链表的中间结点](https://leetcode.cn/problems/middle-of-the-linked-list/)

核心思路：链表存在两种情况。长度为偶数时，快指针指向空，慢指针为中点；长度为奇数时，快指针指向最后一个结点，此时慢指针为中点。



### 环形链表

题目地址：[141. 环形链表](https://leetcode.cn/problems/linked-list-cycle/)

核心思路：如果链表存在环，且因为快指针走的距离永远是慢指针的两倍，所以两个指针一定会相遇。否则就不存在环，快指针会走到终点。



### 环形链表II

题目地址：[142. 环形链表 II](https://leetcode.cn/problems/linked-list-cycle-ii/)

核心思路：假设链表长度为 `a + b` ，当快慢指针第1次相遇时，`f = s + n*b`  此时 `f = 2nb、s = nb`。目标位置为 `a + nb`，现在只需要将快指针移动到头部，每次快慢指针都移动 1 步。当快慢指针第2次相遇时，就是环形入口。



### 重排链表

题目地址：[143. 重排链表](https://leetcode.cn/problems/reorder-list/)

核心思路：观察发现只需要将链表分为两部分来看就容易发现规律，本质上就是两个链表的合并。因此，将任务拆分为找到中点结点，然后对后半链表倒序，最后将两个链表合并就完成任务。
